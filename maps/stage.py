...
class Stage:
    def __init__(self,map,collisions,s_width,s_height,t_size,screen,scroll=True):
        ...
        for i in range(len(frame)):
            for j in range(len(frame[0])):
                pygame.draw.rect(
                    self.screen,
                    frame[i][j],
                    (int((i*self.t_size) - (int(self.scroll)*(player.x % self.t_size))),
                    int((j*self.t_size) - (int(self.scroll)*(player.y % self.t_size))),
                    self.t_size,
                    self.t_size)
                )